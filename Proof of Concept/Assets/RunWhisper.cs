...
public class RunWhisper : MonoBehaviour
{
    ...
    void LoadAudio()
    {
        // 1. Create the target buffer (30 seconds of silence)
        var data = new float[maxSamples];

        // 2. Read the ACTUAL data from the clip
        // We use a temp array because the clip might be shorter than 30s
        float[] clipData = new float[audioClip.samples];
        audioClip.GetData(clipData, 0);

        // 3. Copy the valid data into the 30s buffer
        // This fills the beginning with audio and leaves the rest as 0 (silence)
        int lengthToCopy = Mathf.Min(clipData.Length, data.Length);
        System.Array.Copy(clipData, data, lengthToCopy);
